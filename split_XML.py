    ...
    for subelem in elem:
        ...
        for pictures in PictureInfo:
            ...
            try:
                ...
                for objects in range(len(pictures)):
                    obj_number = len(pictures[1])
                    for properties in range(obj_number): #ИСПРАВИТЬ - НУЖНО ШАГАТЬ ПО ОБЪЕКТАМ В КАЖДОМ OBJECTRECTLIST  
                        if pictures[1][objects][properties].tag == 'ObjectRect':
                            koord = pictures[1][objects][properties].text
            ...
            # ‭ ‬создаём новый файл XML с одним объектом whitehead
            tree = xml.ElementTree(root)
            tree.write(folder + filename.text.replace('jpg', 'xml').strip('""'))
            created_files.append(filename.text.replace('jpg', '').strip('""'))
